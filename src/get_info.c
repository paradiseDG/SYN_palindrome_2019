...
static int simpleflag(int nb_flag, palin_t *info, char *arg)
{
    switch (nb_flag) {
    case 0:
        info->nb = my_atoi(arg);
...
static void getflag(char **av, palin_t *info, int *i)
{
    char flag[5][6] = {"-n\0", "-p\0", "-imin\0", "-imax\0"};
...
palin_t getoptprog(int ac, char **av)
{
    ...
    if ((info.nb == -1 && info.pal == -1) ||
        (info.nb != -1 && info.pal != -1) || info.imin < 0 || info.imax < 0 ||
        info.imin > info.imax)
        exit_comment(2, "Error: bad option\n\0", 84);
    return (info);
}
